...
export class CompanyInterestService {
  ...
  async createCompanyInterest(reqDto: CompanyInterestReqDto) {
    ...
    const savedCategoriesPromise = this.saveInterestCategory(
      reqDto.company_id,
      reqDto.interest_categories,
    );

    // save or update import market
    const savedImportMarketsPromise = this.saveImportMarkets(
      reqDto.company_id,
      reqDto.import_markets,
    );

    // save or update target market
    const savedTargetMarketsPromise = this.saveTargetMarkets(
      reqDto.company_id,
      reqDto.target_markets,
    );

    // save volume
    const savedVolumePromise = this.saveOrderVolume(
      reqDto.company_id,
      reqDto.order_volume_id,
    );
  ...
  private async saveImportMarkets(
    company_id: string,
    import_markets: ImportMarketEntity[] | any[],
  ) {
  ...
  private async saveTargetMarkets(
    company_id: string,
    target_markets: TargetMarketEntity[] | any[],
  ) {
  ...
  private async saveInterestCategory(
    company_id: string,
